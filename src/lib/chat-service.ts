// import { supabase } from "@/integrations/supabase/client";

// // const CHAT_URL = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/chat`;
// const CHAT_URL = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/chat_assistant`;
// type Msg = { role: "user" | "assistant"; content: string };

// export async function streamChat({
//   messages,
//   onDelta,
//   onDone,
//   onError,
// }: {
//   messages: Msg[];
//   onDelta: (deltaText: string) => void;
//   onDone: () => void;
//   onError: (error: string) => void;
// }) {
//   try {
//     const resp = await fetch(CHAT_URL, {
//       method: "POST",
//       headers: {
//         "Content-Type": "application/json",
//         Authorization: `Bearer ${import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY}`,
//       },
//       body: JSON.stringify({ messages }),
//     });

//     if (resp.status === 429) {
//       onError("Too many requests. Please wait a moment and try again.");
//       return;
//     }
//     if (resp.status === 402) {
//       onError("Service temporarily unavailable. Please try again later.");
//       return;
//     }
//     if (!resp.ok || !resp.body) {
//       onError("Failed to get a response. Please try again.");
//       return;
//     }

//     const reader = resp.body.getReader();
//     const decoder = new TextDecoder();
//     let textBuffer = "";
//     let streamDone = false;

//     while (!streamDone) {
//       const { done, value } = await reader.read();
//       if (done) break;
//       textBuffer += decoder.decode(value, { stream: true });

//       let newlineIndex: number;
//       while ((newlineIndex = textBuffer.indexOf("\n")) !== -1) {
//         let line = textBuffer.slice(0, newlineIndex);
//         textBuffer = textBuffer.slice(newlineIndex + 1);

//         if (line.endsWith("\r")) line = line.slice(0, -1);
//         if (line.startsWith(":") || line.trim() === "") continue;
//         if (!line.startsWith("data: ")) continue;

//         const jsonStr = line.slice(6).trim();
//         if (jsonStr === "[DONE]") {
//           streamDone = true;
//           break;
//         }

//         try {
//           const parsed = JSON.parse(jsonStr);
//           const content = parsed.choices?.[0]?.delta?.content as string | undefined;
//           if (content) onDelta(content);
//         } catch {
//           textBuffer = line + "\n" + textBuffer;
//           break;
//         }
//       }
//     }

//     if (textBuffer.trim()) {
//       for (let raw of textBuffer.split("\n")) {
//         if (!raw) continue;
//         if (raw.endsWith("\r")) raw = raw.slice(0, -1);
//         if (raw.startsWith(":") || raw.trim() === "") continue;
//         if (!raw.startsWith("data: ")) continue;
//         const jsonStr = raw.slice(6).trim();
//         if (jsonStr === "[DONE]") continue;
//         try {
//           const parsed = JSON.parse(jsonStr);
//           const content = parsed.choices?.[0]?.delta?.content as string | undefined;
//           if (content) onDelta(content);
//         } catch { /* ignore */ }
//       }
//     }

//     onDone();
//   } catch (e) {
//     console.error("Stream error:", e);
//     onError("Connection error. Please check your internet and try again.");
//   }
// }

// export async function logQuery(query: string, category?: string) {
//   try {
//     await supabase.from("chat_queries").insert({ query: query.slice(0, 1000), category });
//   } catch {
//     // silent fail for analytics
//   }
// }
import { supabase } from "@/integrations/supabase/client";

const CHAT_URL = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/chat_assistant`;

type Msg = { role: "user" | "assistant"; content: string };

export async function streamChat({
  messages,
  onDelta,
  onDone,
  onError,
}: {
  messages: Msg[];
  onDelta: (deltaText: string) => void;
  onDone: () => void;
  onError: (error: string) => void;
}) {
  try {
    const resp = await fetch(CHAT_URL, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        // ❌ removed Authorization (not needed)
      },
      body: JSON.stringify({ messages }),
    });

    if (resp.status === 429) {
      onError("Too many requests. Please wait a moment and try again.");
      return;
    }

    if (!resp.ok) {
      onError("Failed to get a response. Please try again.");
      return;
    }

    const data = await resp.json();

    // ✅ Extract correct content
    const content =
      data?.choices?.[0]?.message?.content || "No response from AI";

    // ✅ Send full response to UI
    onDelta(content);

    onDone();
  } catch (e) {
    console.error("Error:", e);
    onError("Connection error. Please check your internet and try again.");
  }
}

export async function logQuery(query: string, category?: string) {
  try {
    await supabase
      .from("chat_queries")
      .insert({ query: query.slice(0, 1000), category });
  } catch {
    // silent fail
  }
}
